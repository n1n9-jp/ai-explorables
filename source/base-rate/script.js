...
d3.select('#metrics').html('').st({
  ...
  .append('div.metrics-container').st({
    width: 280
  })
  .each(drawMetric)

d3.selectAll('rect.drag')
  .on('mouseover.style', d => d3.selectAll('rect.' + d).st({
    strokeWidth: 3,
    stroke: '#000'
  }))
  .on('mouseout.style', d => d3.selectAll('rect.' + d).st({
    strokeWidth: 0
  }))
...
function drawMetric(i) {
  ...
  var text = [
    ...
    '検査陽性者のうち<br>病気になった人の割合',
    '実際に病気になった人のうち<br>検査陽性者の割合',
    '検査陰性者のうち<br>健康な人の割合',
  ][i]
  ...
  sel.append('h3').st({
    marginBottom: 20,
    fontSize: isScaled ? 30 : 20
  }).html(isScaled ? text.replace('<br>', '') : text)

  var h = 200
  var width = 100

  var fDiv = sel.append('div').st({
      position: 'relative',
      top: -h + 7
    })
    .datum({
  ...
  mSel.append('circle').at({
    r: 4,
    cx: width / 2 + .5,
    fill: 'none',
    stroke: '#000'
  })
  fSel.append('circle').at({
    r: 4,
    cx: width / 2 + .5,
    fill: 'none',
    stroke: '#000'
  ...
  renderFns.push(() => {
    ...
    errorSel.at({
      d: 'M 0 ' + (h - h * fPercent) + ' V ' + (h - h * mPercent)
    })
  })
}
...
function drawMatrix({
  ...
  rectSel.filter(d => colors[d.i] == '#fff').at({
    fill: '#eee'
  })

  var bh = .5
  svg.append('rect.tpr').at({
      height: bh
    }).translate(-bh / 2, 1)
    .datum('tpr')

  svg.append('rect.fnr').at({
      height: bh
    }).translate(-bh / 2, 1)
    .datum('fnr')

  svg.append('rect.b').at({
      width: bh,
      height: w
    }).translate(-bh / 2, 0)
    .datum('b')
  ...
  function ttFormat(str) {
    var html = ''
    if (str == 'tpr') html = `病気な${s.titleStr.toLowerCase()}の${d3.format('.0%')(s.tpr)}は検査陽性です`
    if (str == 'fnr') html = `健康な${s.titleStr.toLowerCase()}の${d3.format('.0%')(s.fnr)}は検査陰性です`
    if (str == 'b') html = `${s.titleStr.toLowerCase()}の${d3.format('.0%')(s.b)}は病気です`
    ttSel.html(html)
  }
  ...
  svg.append('text').text(s.titleStr).at({
    textAnchor: 'middle',
    x: w / 2,
    y: -8,
    fontSize: 20
